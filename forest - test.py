...
class Character:
    ...
    instead of lists so that the item.name can be displayed, but 
    """
...
def attack():
    ...
    while player.hp > 0 and monster_attacked.hp > 0:
        ...
        if monster_attacked.attack > 18: mobattackstring = colorama.Fore.RED + colorama.Style.BRIGHT + "HARD!!"
        ...
        if player.attack > 18: playerattackstring = colorama.Fore.RED + colorama.Style.BRIGHT + "HARD!!"
...
def look(argument=0):
    ...
    for item in player.inventoryList:
        invlist.append(item.name)
    eqlist = []
    for item in player.equippedList:
        eqlist.append(item.name)
    if argument == 0:
        setting()
    elif argument in invlist:
        item = invlist.index(argument)
        print(colorama.Style.RESET_ALL + item.name)
