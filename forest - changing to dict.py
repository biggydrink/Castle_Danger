...
def prompt():
    print("<" + currentroom.getName() + " : " + str(player.hp) + "HP : " + currentroom.getExits() + ">", end="")
...
#                prompt()
        if commandlist[0] in commandsDict:
            commandsDict.get(commandlist[0])()
            prompt()
...
def attack():
    ...
    while player.hp > 0 and monster_attacked.hp > 0:
        ...
        if player.attack > 18:
            playerattackstring = "HARD!!"
        elif player.attack > 14:
            playerattackstring = "omg so hard"
        elif player.attack > 12:
            playerattackstring = "...ouch!"
        elif player.attack > 10:
            playerattackstring = "pretty hard."
        elif player.attack > 7:
            playerattackstring = "kinda hard."
        elif player.attack > 5:
            playerattackstring = "kinda."
        else:
            playerattackstring = "a little bit."
...
def equip(item):
    if item in player.inventoryList:
        player.equippedList.append(item)
        player.hp += item.hp
...
def unequip(item):
    if item in player.equippedList:
